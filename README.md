...
Q: What Java version does MariaDB4j require?
A: MariaDB4j requires at least [Java 17 (LTS)](https://en.wikipedia.org/wiki/Java_version_history#Java_SE_17) currently, since [v3.0.0](https://github.com/MariaDB4j/MariaDB4j/issues/650). More recent Java versions (e.g. 21 LTS) are also supported, and [any forward incompatibility should be reported as a bug](https://github.com/MariaDB4j/MariaDB4j/issues/903). But there is no backwards compatibility with earlier Java versions anymore. (MariaDB4j v2.6.0 required at least Java 11 instead of 17, and v2.5.3 still worked on the now ancient Java 8 only - but both have known bugs, and are not supported anymore.) Please [💸 sponsor 🫶](#sponsors) maintenance, or request (paid for) individual support for custom builds with compatibility to older Java versions.

Q: `java.lang.UnsupportedClassVersionError: ch/vorburger/mariadb4j/MariaDB4jService has been compiled by a more recent version of the Java Runtime (class file version 61.0), this version of the Java Runtime only recognizes class file versions up to 55.0 at java.base/java.lang.ClassLoader.defineClass1(Native Method)`
A: You need a more recent Java version than the one you are running on to be able to use MariaDB4j; see above.
